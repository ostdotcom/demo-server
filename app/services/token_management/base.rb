module TokenManagement
  ...
  class Base
    ...
    def validate_token_id
      Validator.is_integer?(@token_id) ?
        Result.success({}) :
        Result.error('a_s_tm_b_1', 'INVALID_REQUEST', 'Invalid token id')
    end

    # Validate API key
    #
    def validate_api_key
      Validator.is_alphanumeric?(@api_key) ?
        Result.success({}) :
        Result.error('a_s_tm_b_2', 'INVALID_REQUEST', 'Invalid api key')
    end

    # Validate API secret
    #
    def validate_api_secret
      Validator.is_alphanumeric?(@api_secret) ?
        Result.success({}) :
        Result.error('a_s_tm_b_3', 'INVALID_REQUEST', 'Invalid api secret')
    end

    # Validate token name
    #
    def validate_token_name
      Validator.is_alphanumeric_space?(@name) ?
        Result.success({}) :
        Result.error('a_s_tm_b_4', 'INVALID_REQUEST', 'Invalid token name')
    end

    # Validate token name
    #
    def validate_token_symbol
      Validator.is_alphanumeric?(@symbol) ?
        Result.success({}) :
        Result.error('a_s_tm_b_5', 'INVALID_REQUEST', 'Invalid token symbol')
    end

    # Validate token url id
    #
    def validate_url_id
      Validator.is_alphanumeric?(@url_id) ?
        Result.success({}) :
        Result.error('a_s_tm_b_6', 'INVALID_REQUEST', 'Invalid token url id')
    end

    # Validate PC token holder uuid
    #
    def validate_token_holder_uuid
      Validator.is_uuid_v4?(@pc_token_holder_uuid) ?
        Result.success({}) :
        Result.error('a_s_tm_b_7', 'INVALID_REQUEST', 'Invalid token holder uuid')
    end
    ...
    def validate_token_details
      ...
      if token_details['id'] != @token_id || token_details['name'] != @name || token_details['symbol'] != @symbol
